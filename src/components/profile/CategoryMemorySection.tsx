  ...
  return <div className="bg-white rounded-lg border shadow-sm overflow-hidden">
    ...
    <div className="space-y-3 p-4 min-h-[420px]">
      {categories.map((category) => {
        const isOpen = selected?.id === category.id;
        return <details key={category.id} open={isOpen} className="rounded-lg border bg-white">
          <summary className={`cursor-pointer px-4 py-3 font-medium text-gray-900 flex items-center gap-2 ${isOpen ? 'bg-blue-50 text-blue-800' : ''}`} onClick={(event) => { event.preventDefault(); setSelected(isOpen ? null : category); }}>
            <FolderOpen size={16}/><span className="flex-1 truncate">{category.name}</span><ChevronRight size={14} className={`transition-transform ${isOpen ? 'rotate-90' : ''}`}/>
          </summary>
          {isOpen && selected && <section className="border-t p-5 space-y-4">
