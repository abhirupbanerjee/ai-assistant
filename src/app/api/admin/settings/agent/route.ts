...
export async function GET() {
  try {
    ...
    const settings = {
      ...
      budgetMaxLlmCalls: Number.parseInt(await getSetting('agent_budget_max_llm_calls', '500'), 10),
      budgetMaxTokens: Number.parseInt(await getSetting('agent_budget_max_tokens', '2000000'), 10),
      budgetMaxWebSearches: Number.parseInt(await getSetting('agent_budget_max_web_searches', '100'), 10),
      confidenceThreshold: Number.parseInt(await getSetting('agent_confidence_threshold', '80'), 10),
      ...
      budgetMaxDurationMinutes: Number.parseInt(await getSetting('agent_budget_max_duration_minutes', '30'), 10),
      taskTimeoutMinutes: Number.parseInt(await getSetting('agent_task_timeout_minutes', '5'), 10),
      // Retry reserve
      budgetRetryReserveLlmCalls: Number.parseInt(await getSetting('agent_budget_retry_reserve_llm_calls', '10'), 10),
      budgetRetryReserveTokens: Number.parseInt(await getSetting('agent_budget_retry_reserve_tokens', '50000'), 10),
      ...
      hitlMinTasks: Number.parseInt(await getSetting('agent_hitl_min_tasks', '5'), 10),
      hitlTimeoutSeconds: Number.parseInt(await getSetting('agent_hitl_timeout_seconds', '300'), 10),
  ...
  try {
    ...
    return NextResponse.json({
      ...
      settings: {
        ...
        subagentMaxIterations: Number.parseInt(await getSetting('agent_subagent_max_iterations', '5'), 10),
        subagentBudgetRatio: Number.parseInt(await getSetting('agent_subagent_budget_ratio', '25'), 10),
