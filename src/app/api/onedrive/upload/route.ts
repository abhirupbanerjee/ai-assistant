/**
 * OneDrive upload bridge.
 *
 * POST /api/onedrive/upload
 * Uploads a generated artifact to the authenticated user's OneDrive.
 *
 * Mode A — server-side artifact:
 *   { outputId: number, context: 'thread' | 'workspace', folderName?, filename? }
 *
 * Mode B — client-rendered artifact:
 *   { filename: string, mimeType: string, contentBase64: string, folderName? }
 *
 * OneDrive does not support Google's "convert to native format" option, so the
 * `convertToGoogleFormat` prop is intentionally ignored here.
 */
...
/**
 * Resolve the upload MIME type. Prefer the filename extension, then fall back
 * to the coarse file_type mapping.
 */
...
export async function POST(request: NextRequest) {
  ...
  // Pre-flight: without a connected Microsoft account the connector would fall
  // back to the service-account / app identity and the file would not land in
  // the user's personal OneDrive — fail fast instead.
  try {
    const connected = await hasConnectedAccount(user.email, 'microsoft');
    if (!connected) {
      return NextResponse.json(
        { error: 'Microsoft account not connected. Connect it in Settings to save to OneDrive.', code: 'RECONNECT_REQUIRED' },
        { status: 401 }
      );
    }
  } catch {
  ...
  const url = `${CONNECTOR_BASE_URL}/ms_drive_upload_file`;
  ...
  try {
    ...
    if (!response.ok || !data.ok) {
      ...
      console.warn('[onedrive/upload] connector error', { code, status: data.status ?? response.status });
      const status =
        code === 'RECONNECT_REQUIRED' || code === 'INSUFFICIENT_SCOPE'
          ...
          : data.status && data.status >= 400
            ? data.status
            : response.status || 502;
      return connectorError(status, data.error || 'OneDrive upload failed', code);
    }
    ...
    return NextResponse.json(
      {
        error: err instanceof Error ? err.message : 'Failed to upload to OneDrive',
