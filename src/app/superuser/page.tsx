...
const VALID_TABS: TabType[] = ['dashboard', 'categories', 'users', 'documents', 'prompts', 'tools', 'skills', 'workspaces', 'agents', 'settings'];
...
function SuperUserPageContent() {
  ...
  useEffect(() => {
    ...
    // Legacy URL redirect: 'agent' was a top-level tab, now promoted to 'agents' L1
    if (tab === 'agent') {
      setActiveTab('agents');
      router.replace('/superuser?tab=agents', { scroll: false });
  ...
  // Settings sidebar section state (SettingsSection imported from SuperuserSidebarMenu)
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-50">
      ...
      <div className="flex flex-1 min-h-0">
        ...
        {/* Agents Section (level 1 menu item — view only) */}
        {activeTab === 'agents' && (
        ...
        {activeTab === 'settings' && (
          <>
            ...
            {/* Autonomous Mode (view only — L2 sibling of LLM, positioned after it) */}
            {settingsSection === 'autonomous-mode' && <AgentSettingsTab readOnly />}

            {/* Agent Registry (view only — L2 sibling of LLM, positioned after Autonomous Mode) */}
            {settingsSection === 'agent-registry' && <AgentRegistryTab readOnly />}
