...
export async function ragQuery(
  ...
  if (categoryIds.length > 0) {
    ...
    if (isKbOverviewQuery) {
      try {
        ...
        logger.debug('KB overview prepared', {
          categoryIds,
          docCount: summaries.length,
          contextLength: kbOverviewContext.length,
        });
      } catch (err) {
        logger.warn('KB overview detection failed', { error: String(err) });
      }
    }
  }
  ...
  // chunk-based context with the synthesized category overview. This bypasses
  // the similarity search and reranker results entirely — the KB overview is
  // built from pre-computed per-document summaries, not from chunk retrieval.
  if (kbOverviewContext) {
    context = kbOverviewContext;
    logger.debug('Using KB overview context instead of chunk-based context', {
      contextLength: kbOverviewContext.length,
    });
  }
