...
export class DocxBuilder {
  ...
  async generate(): Promise<DocxResult> {
    ...
    const doc = new Document({
      creator: this.options.metadata?.author || this.branding.organizationName || 'AI Assistant',
  ...
  private createDataTable(rows: string[][]): Table | Paragraph {
    ...
    // Calculate even column widths so Word/Google Docs render columns at the
    // proper width instead of collapsing them to a narrow vertical strip.
