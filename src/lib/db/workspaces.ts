...
function rowToWorkspace(row: WorkspaceRow): Workspace {
  return {
    ...
    sources_enabled: row.sources_enabled === 1,
  ...
  return transaction(() => {
    execute(
      `INSERT INTO workspaces (
        ...
        voice_enabled, file_upload_enabled, max_file_size_mb, web_search_enabled, sources_enabled, auth_required,
      ...
      [
        ...
        (input.sources_enabled ?? true) ? 1 : 0,
...
export function updateWorkspace(id: string, updates: UpdateWorkspaceInput): Workspace | null {
  ...
  if (updates.sources_enabled !== undefined) {
    setClauses.push('sources_enabled = ?');
    params.push(updates.sources_enabled ? 1 : 0);
  }
