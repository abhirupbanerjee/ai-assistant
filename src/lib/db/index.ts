...
function runMigrations(database: Database.Database): void {
  ...
  // Migration: Update file_type CHECK constraint to include 'html' format for HTML document generation
  try {
    // Test if 'html' is allowed by the current constraint
    database.exec(`
      INSERT INTO thread_outputs (thread_id, filename, filepath, file_type, file_size)
      VALUES ('__migration_test_html__', '__test__', '__test__', 'html', 0)
    `);
    // If successful, delete the test row
    database.exec(`DELETE FROM thread_outputs WHERE thread_id = '__migration_test_html__'`);
  } catch {
    // 'html' is not allowed, need to recreate the table with updated constraint
    database.exec(`
      BEGIN IMMEDIATE;
      -- Drop any leftover temp table from interrupted migration
      DROP TABLE IF EXISTS thread_outputs_new;
      -- Create new table with updated constraint (includes html for HTML document generation)
      CREATE TABLE thread_outputs_new (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        thread_id TEXT NOT NULL,
        message_id TEXT,
      ...
      COMMIT;
    `);
    console.log('[DB Migration] Added html file type to thread_outputs');
  }

    // Migration: Update file_type CHECK constraint to include 'zip' format for site_gen website packaging
    // Also repair any site_gen rows previously mislabeled as 'html' (filename ends in .zip)
    try {
      database.exec(`
        UPDATE thread_outputs SET file_type = 'zip' WHERE file_type = 'html' AND filename LIKE '%.zip'
      `);
    } catch {
    ...
    try {
      database.exec(`
        INSERT INTO thread_outputs (thread_id, filename, filepath, file_type, file_size)
        VALUES ('__migration_test_zip__', '__test__', '__test__', 'zip', 0)
      `);
      database.exec(`DELETE FROM thread_outputs WHERE thread_id = '__migration_test_zip__'`);
    } catch {
      console.log('[DB Migration] Adding zip support to thread_outputs...');
      database.exec(`
        BEGIN TRANSACTION;
        -- Drop any leftover temp table from interrupted migration
        DROP TABLE IF EXISTS thread_outputs_new;
        -- Create new table with updated constraint (includes zip for site_gen website packaging)
        CREATE TABLE thread_outputs_new (
          id INTEGER PRIMARY KEY AUTOINCREMENT,
          thread_id TEXT NOT NULL,
          message_id TEXT,
        ...
        COMMIT;
      `);
      console.log('[DB Migration] Added zip file type to thread_outputs');
    }
  ...
  if (workspaceOutputsExists) {
    ...
    // Check if html migration is needed
    const tableInfoAfterWav = database.prepare(
      "SELECT sql FROM sqlite_master WHERE type='table' AND name='workspace_outputs'"
    ).get() as { sql: string } | undefined;

    if (tableInfoAfterWav && !tableInfoAfterWav.sql.includes("'html'")) {
      console.log('[DB Migration] Adding html support to workspace_outputs...');

      database.exec(`
        -- Create new table with updated CHECK constraint (includes html for HTML document generation)
        CREATE TABLE workspace_outputs_new (
          id INTEGER PRIMARY KEY AUTOINCREMENT,
          workspace_id TEXT NOT NULL,
          session_id TEXT NOT NULL,
      ...
      console.log('[DB Migration] Added html support to workspace_outputs');
    }

    // Migration: Update file_type CHECK constraint to include 'zip' format for site_gen website packaging
    // Also repair any site_gen rows previously mislabeled as 'html' (filename ends in .zip)
    const tableInfoAfterHtml = database.prepare(
      "SELECT sql FROM sqlite_master WHERE type='table' AND name='workspace_outputs'"
    ).get() as { sql: string } | undefined;

    if (tableInfoAfterHtml && !tableInfoAfterHtml.sql.includes("'zip'")) {
      console.log('[DB Migration] Adding zip support to workspace_outputs...');
      try {
        database.exec(`
          UPDATE workspace_outputs SET file_type = 'zip' WHERE file_type = 'html' AND filename LIKE '%.zip'
        `);
      } catch {
        // ignore update failure
      }
      database.exec(`
        BEGIN TRANSACTION;
        -- Create new table with updated CHECK constraint (includes zip for site_gen website packaging)
        CREATE TABLE workspace_outputs_new (
          id INTEGER PRIMARY KEY AUTOINCREMENT,
          workspace_id TEXT NOT NULL,
          session_id TEXT NOT NULL,
      ...
      console.log('[DB Migration] Added zip support to workspace_outputs');
    }
  }
  ...
  // has kb_read but is missing kb_search (agents seeded before kb_search existed).
  // Idempotent. Completes the kb_* ladder (kb_summary → kb_search → kb_read).
  database.exec(`
    UPDATE agent
    SET tool_allowlist = json_insert(tool_allowlist, '$[#]', 'kb_search')
    WHERE tool_allowlist IS NOT NULL
      AND tool_allowlist LIKE '%"kb_read"%'
  ...
  // free to call the tool but is not forced. Idempotent via INSERT OR IGNORE on
  // deterministic seed ids. Admins can edit/delete these from the Tool Routing UI.
  database.exec(`
    INSERT OR IGNORE INTO tool_routing_rules (id, tool_name, rule_name, rule_type, patterns, force_mode, priority, category_ids, is_active, created_by, updated_by)
    VALUES
  ...
  `);
  console.log('[DB Migration] Seeded suggested kb_* tool-routing rules (idempotent)');
