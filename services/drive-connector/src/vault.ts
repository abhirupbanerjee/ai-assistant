 ...
 *
 * Flow:
 *   1. `getUserToken(cfg, userId)` calls `GET {APP_BASE_URL}/api/connectors/vault/tokens`
 ...
 *   4. On 404 (no connected account), returns null → caller falls back to the
 *      shared service-account identity (Phase 1 behavior).
 *   5. On 409 (RECONNECT_REQUIRED), returns a `ReconnectRequired` sentinel so
 *      the caller can surface a structured error to the LLM (§8 Task 6).
 *
 * Security:
 ...
 * The caller should surface a RECONNECT_REQUIRED error to the LLM.
 */
export const RECONNECT_REQUIRED = Symbol('RECONNECT_REQUIRED');

/** Result of a vault lookup: a token, null (fall back to service account), or reconnect. */
 ...
 * Uses the same shared secret as the app-side injection (§8 Task 3).
 */
 ...
 *
 * Returns:
 *   - `VaultToken` — a valid per-user token (caller should use it)
 *   - `null` — no connected account for this user/provider → fall back to
 *     the shared service-account identity (Phase 1 behavior)
 *   - `RECONNECT_REQUIRED` — the account exists but needs reconnection
 *     (caller should surface a RECONNECT_REQUIRED error to the LLM)
 */
export async function getUserToken(
  ...
  // Phase 1 fallback: if the vault is not configured, immediately return null.
  ...
  // Check negative cache (avoids repeated 404s for unconnected users).
  ...
  let data: unknown;
  try {
    data = await getJson(url, headers, cfg.googleTimeoutMs);
  } catch (err) {
    if (err instanceof HttpError) {
      if (err.status === 404) {
        // No connected account — cache the negative result and fall back.
        negativeCache.set(cacheKey, now + NEGATIVE_CACHE_MS);
        logger.info('Vault: user not connected, falling back to service account', { userId, provider });
        return null;
      }
      if (err.status === 409) {
        // Account exists but token is missing — reconnect needed.
        logger.warn('Vault: reconnect required', { userId, provider });
        return RECONNECT_REQUIRED;
      }
      // Other errors (401, 403, 500, etc.) — log and fall back rather than
      // failing the entire tool call. The service account can still serve.
      logger.error('Vault endpoint error, falling back to service account', {
        userId,
        provider,
        status: err.status,
    ...
    logger.error('Vault request failed, falling back to service account', {
      userId,
      provider,
  ...
  if (!resp.accessToken) {
    logger.warn('Vault returned no access token, falling back', { userId, provider });
    return null;
  }
...
/**
 * Invalidate the cached per-user token (e.g. on a 401 from Google).
 * The next call will re-fetch from the vault.
 */
