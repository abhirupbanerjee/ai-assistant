/**
 * HTTP server entry point for the Slack connector microservice.
 *
 * Endpoints:
 *   GET  /health   — liveness probe (no auth)
 *   GET  /tools    — 5 Slack tool schemas in OpenAI function format
...
async function handleRequest(
  ...
  if (method === 'GET' && path === '/health') {
    sendJson(req, res, cfg, 200, { ok: true, service: 'slack-connector' });
    return;
  }
...
export function startServer(cfgOverride?: AppConfig): http.Server {
  ...
  server.listen(cfg.port, () => {
    logger.info('slack-connector listening', { port: cfg.port, logLevel: cfg.logLevel });
  });
