 ...
 *
 * This file is 100% reusable across all connector microservices.
 * Copy as-is into your connector's src/ directory.
 */
...
export interface HttpRequestOptions {
  ...
  body?: string | Buffer;
  timeoutMs: number;
  /** Expected JSON response — parses automatically. */
  json?: boolean;
}
...
export interface HttpResponse {
  ...
  /** Raw text body. */
  text: string;
  /** Parsed JSON if `json: true` and body parses. */
  data?: unknown;
}
        ...
        res.on('end', () => {
          ...
          if (opts.json) {
            try {
              ...
              // leave data undefined; caller can inspect text
            }
          }
          if (out.status >= 400) {
...
/** Convenience: GET expecting JSON. */
...
/** Convenience: POST JSON expecting JSON. */
