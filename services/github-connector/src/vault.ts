/**
 * Token vault — fetches per-user GitHub OAuth tokens from the AI-assistant app.
 *
 * Copied from services/_connector-template/src/vault.ts
 * VaultProvider extended with 'github'.
 */
...
export async function getUserToken(
  ...
  let data: unknown;
  try {
    data = await getJson(url, headers, cfg.githubTimeoutMs);
  } catch (err) {
  ...
  if (resp.tokenExpiry) {
    ...
    // GitHub tokens never expire — use a very long cache TTL.
